...
//Prompts if user selects to view and edit employee
...
//Prompts if user selects to view and edit department
...
//Prompts if user selects to view and edit roles
const rolesPrompt = () => {
        return inquirer.prompt({
                type: 'list',
                name: 'rolesMenu',
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//get user input about departments
const inputDepName = () => {
        return inquirer.prompt({
                type: 'input',
                name: 'depName',
...
//>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
//get user input about employees
const inputEmplInfo = () => {
        return inquirer.prompt([
                {
                type: 'input',
